...
cv::Mat drawepilines(std::vector<cv::Vec3f> epilinepoints,cv::Mat& img1, std::vector<cv::Point2f> imagecoordinates) {
	...
	std:: vector<double> projection_error_list;
	...
	for (int i = 0; i<epilinepoints.size();i++) {
		...
		projection_error_list.push_back(projection_error);
	}
	// The output generated is in yml format and sublime text or notepad can be used to open this file
	cv::FileStorage fs("Projection Error.yml", cv::FileStorage::WRITE);
	fs << "Projection Error" << projection_error_list;
	fs.release();
	return img1;
}
...
int main() {
	...
	// The output generated is in yml format and sublime text or notepad can be used to open this file
	cv::FileStorage fs("Fundamental Matrix.yml", cv::FileStorage::WRITE);
	fs << "Fundamental Matrix" << fund_matrix;
	fs.release();
